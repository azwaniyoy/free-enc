    ...
    hisoka.ev.on('group-participants.update', async (anu) => {
        ...
        try {
		   ...
		   var but = [{buttonId: `/`, buttonText: { displayText: "Welcome 🥳" }, type: 1 }]
                    hisoka.sendMessage(anu.id, { caption: `Hallo @${num.split("@")[0]} Selamat Datang Di Grup *${metadata.subject}*\nSilahkan Untuk Memperkenalkan diri anda`, image: {url: ppuser}, buttons: but, footer: `©Message : ${metadata.subject}`, mentions: [num]})
                } else if (anu.action == 'remove') {
                   var but = [{buttonId: `/`, buttonText: { displayText: "Selamat Tinggal Beban 👋" }, type: 1 }]
                    hisoka.sendMessage(anu.id, { image: { url: ppuser }, buttons: but, contextInfo: { mentionedJid: [num] }, caption: `@${num.split("@")[0]} Leaving To ${metadata.subject}` })
                }
            }
        } catch (err) {
            console.log(err)
